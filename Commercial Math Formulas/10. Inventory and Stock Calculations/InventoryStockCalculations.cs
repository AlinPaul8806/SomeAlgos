...
namespace CommercialMathFormulas.InventoryStockCalculations
{
    public class InventoryStockCalculations
    {
        ...
        public static decimal ReorderPoint(decimal avgDailyUsage, int leadTimeDays, decimal safetyStock)
        {
            return (avgDailyUsage * leadTimeDays) + safetyStock;
        }

        // Safety stock based on demand variability and service level (simplified).
        public static decimal SafetyStock(decimal stdDevDailyDemand, int leadTimeDays, decimal serviceFactor)
        {
        ...
        public static decimal InventoryTurnover(decimal cogs, decimal avgInventory)
        {
            if (avgInventory == 0) 
            { 
        ...
        public static decimal DaysOfInventory(decimal avgInventory, decimal cogs)
        {
            if (cogs == 0)
            {
                throw new DivideByZeroException();
